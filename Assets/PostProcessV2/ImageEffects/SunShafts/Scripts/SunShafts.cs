...
namespace PostProcessV2.ImageEffects
{
    [ExecuteInEditMode]
    ...
    public class SunShafts : PostEffectsBase
    {
        ...
        public Shader prepareSunShaftsBlurShader;
        private Material prepareSunShaftsBlurMaterial;
        ...
        {
            prepareSunShaftsBlurShader = Shader.Find("Hidden/PrepareSunShaftsBlur");
            simpleClearShader = Shader.Find("Hidden/SimpleClear");                
            sunShaftsShader = Shader.Find("Hidden/SunShaftsComposite");

            if (prepareSunShaftsBlurShader == null)
            {
                Debug.LogError("Error in starting the Sun Shafts effect, could not find the shader: Hidden/PrepareSunShaftsBlur");
                Destroy(this);
            }

            if (simpleClearShader == null)
            {
                Debug.LogError("Error in starting the Sun Shafts effect, could not find the shader: Hidden/SimpleClear");
                Destroy(this);
            }

            if (sunShaftsShader == null)
            {
                Debug.LogError("Error in starting the Sun Shafts effect, could not find the shader: Hidden/SunShaftsComposite");
                Destroy(this);
            }
        ...
        public override bool CheckResources() 
        {
            ...
            prepareSunShaftsBlurMaterial = CheckShaderAndCreateMaterial(prepareSunShaftsBlurShader, prepareSunShaftsBlurMaterial);
            ...
            {
                Debug.LogWarning("Unable to find the Directional Light component, thus disabling Sun Shafts");
                enabled = false;
            }
